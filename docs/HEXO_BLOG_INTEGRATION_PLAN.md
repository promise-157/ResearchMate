...
R1 的发现、补全、审核、保存规则、源码、确定性排序和候选简报均存在完整后端路径，181 项后端/桌面
测试（其中 1 项受限沙箱 skip）及 17 项离线 Playwright 在完成时通过；E 的失败不推进检查点、归档/清空
及 F3 的结构/配置失败审计都有代码和单元证据。真实网络验收范围与文档一致，F3 未调用真实 AI。

“R1-A–R1-F 已完成”的核心产品结论成立，但以下声明需在博客实施前收紧：

- F2 API/文档允许 50 条，统一页面选择器实际限制为 20 条；必须统一为真实产品上限。
- `reloadWorkspace()` 未清空 `rankingById`，不同工作区复用候选 ID 时可能显示旧分数。
- `Promise.allSettled` 在局部加载失败时保留旧数组；工作区切换后规则或简报请求失败可能短暂展示上一
  工作区数据。应在 generation/切换开始时清空，普通刷新仍可保留成功区域。
- `candidate_ai_runs` 已接入启动恢复，但缺少直接断言 running -> failed 的测试；浏览器也只覆盖简报成功，
  未直接覆盖失败历史和跨工作区简报隔离。
- 架构文档的通用启动恢复列表尚未列出 `candidate_ai_runs`，与后文“恢复已覆盖”不完全一致。

这些是完成证据和隔离一致性的修正，不是新增 R1 能力。
