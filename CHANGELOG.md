...
- Closed the targeted R1 consistency audit without expanding the radar: deterministic ranking is uniformly bounded to 1–20 candidates across UI, request schema, service and docs; workspace changes clear radar jobs, candidates, rules, briefs and local scores before partial reloads; startup recovery directly proves interrupted candidate briefs become visible failures; and offline browser coverage now includes failed brief history, same-ID candidates and failed partial loads across workspaces.
